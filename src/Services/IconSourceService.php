...
class IconSourceService
{
    ...
    private function initialize_sources(): void
    {
         ...
         */
        $iconSources = apply_filters('jooosi-icon/service/icon:sources', ['jooosi' => ['name' => 'Bundled Jooosi Icons', 'path' => $this->svg_dir, 'url' => $this->svg_url]]);
        ...
        $iconSetPaths = array_map(static fn(array $source): string => $source['path'], $this->icon_sources);
        $iconSetPaths['omni'] = $this->icon_sources['jooosi']['path'];
        $this->registry = new LocalSvgIconRegistry($this->svg_dir, $iconSetPaths, ['omni:omni-icon' => 'jooosi:jooosi-icon']);
    }
    /**
    ...
    {
        return $this->get_icon_sets()['jooosi'] ?? ['name' => 'Bundled Jooosi Icons', 'total' => 0, 'samples' => []];
    }
    /**
    ...
    public function get_icon_sets(): array
    {
        ...
        return $this->cache->get($cache_key, function (ItemInterface $item) {
            ...
            foreach ($this->icon_sources as $prefix => $source) {
                ...
                $sets[$prefix] = ['name' => $source['name'], 'total' => count($files), 'samples' => array_map(static fn(string $file): string => pathinfo(basename($file), \PATHINFO_FILENAME), $files)];
            }
            return $sets;
        });
    }
        ...
        return $this->cache->get($cache_key, function (ItemInterface $item) {
            ...
            foreach ($this->icon_sources as $prefix => $source) {
                ...
                foreach ($files as $file) {
                    ...
                    $icons[] = ['name' => $name, 'filename' => $filename, 'icon_name' => "{$prefix}:{$name}", 'prefix' => $prefix, 'url' => $this->get_icon_url($source, $filename), 'path' => $file];
                }
            }
            return $icons;
        });
    ...
    public function search_icons(string $query): array
    {
        ...
        return array_values(array_map(static fn(array $icon): array => ['name' => $icon['name'], 'prefix' => $icon['prefix']], array_filter($all_icons, static fn(array $icon): bool => ($prefix === '' || $icon['prefix'] === $prefix) && str_contains($icon['name'], $search_term))));
    }
    /**
    ...
    {
        $defaultSource = ['name' => 'Bundled Jooosi Icons', 'path' => $this->svg_dir, 'url' => $this->svg_url];
        if (!is_array($sources)) {
            return ['jooosi' => $defaultSource];
        }
        $normalized = ['jooosi' => $defaultSource];
        foreach ($sources as $prefix => $source) {
            if (!is_string($prefix) || !preg_match('/^[a-z0-9]+(?:-[a-z0-9]+)*$/', $prefix) || in_array($prefix, ['local', 'omni'], \true) || !is_array($source) || !isset($source['path']) || !is_string($source['path'])) {
                continue;
            }
            ...
            $normalized[$prefix] = ['name' => $name, 'path' => rtrim($source['path'], '/'), 'url' => rtrim($url, '/')];
        }
        return $normalized;
    }
    /**
