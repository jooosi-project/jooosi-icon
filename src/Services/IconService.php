...
class IconService
{
    ...
    public function __construct(private \JooosiIcon\Services\LocalIconService $localIconService, private \JooosiIcon\Services\IconSourceService $iconSourceService, private \JooosiIcon\Services\IconifyService $iconifyService, private LoggerService $logger, private \JooosiIcon\Services\SvgSanitizer $svgSanitizer)
    {
    }
    /**
    ...
    private function get_or_create_registry(): IconRegistryInterface
    {
        if (!$this->registry instanceof IconRegistryInterface) {
            ...
            $this->registry = new ChainIconRegistry([
                $this->localIconService->get_registry(),
                // Check local uploaded icons first
                $this->iconSourceService->get_registry(),
                // Check built-in and third-party sources next
                $this->iconifyService->get_registry(),
            ]);
        }
    ...
    public function get_icon(string $name, array $attributes = []): ?string
    {
        ...
        try {
            ...
            $svg = $icon->toHtml();
            $skipSanitization = apply_filters('jooosi-icon/service/icon:skip_render_sanitization', \false, $name, $attributes);
            $skipSanitization = apply_filters_deprecated('omni-icon/service/icon:skip_render_sanitization', [$skipSanitization, $name, $attributes], JOOOSI_ICON::VERSION, 'jooosi-icon/service/icon:skip_render_sanitization');
            if ($skipSanitization) {
                return $svg;
            }
            ...
            $this->logger->error('Failed to get icon', ['component' => LogComponent::ICON_SERVICE, 'exception' => $e, 'icon' => $name]);
            return null;
        }
    }
    /**
    ...
    public function get_icon_data(string $name): ?array
    {
        ...
        return ['svg' => $svg, 'name' => $icon, 'prefix' => $prefix];
    }
    /**
    ...
    public function search_icons(string $query): array
    {
        ...
        return ['results' => $allResults, 'total' => count($allResults)];
    }
}
