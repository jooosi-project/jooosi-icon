...
final class SvgSanitizer
{
     ...
     */
    private const SAFE_ANIMATION_TARGETS = [
        'alignment-baseline',
        'baseline-shift',
        'clip-path',
        'clip-rule',
    ...
    public function __construct()
    {
        ...
        $this->sanitizer->setAllowedTags(new SvgSanitizerAllowedTags());
        $this->sanitizer->setAllowedAttrs(new SvgSanitizerAllowedAttributes());
    ...
    private function removeUnsafeAnimations(string $svg): string|false
    {
        ...
        foreach (iterator_to_array($elements, false) as $element) {
            ...
            if (in_array($tagName, self::ANIMATION_ELEMENTS, true)
                && $this->hasUnsafeAnimation($element, $tagName)
            ) {
                $element->parentNode?->removeChild($element);
            }
        }
    ...
    private function hasUnsafeAnimation(DOMElement $animation, string $tagName): bool
    {
        ...
        if (in_array($tagName, ['animate', 'animatecolor', 'animatetransform', 'set'], true)) {
            if ($target === '' || !in_array($target, self::SAFE_ANIMATION_TARGETS, true)) {
                return true;
            }
        } elseif ($target !== '' && !in_array($target, self::SAFE_ANIMATION_TARGETS, true)) {
            return true;
        }

        $attributeType = strtolower(trim($animation->getAttribute('attributeType')));
        if ($attributeType !== '' && !in_array($attributeType, ['auto', 'css', 'xml'], true)) {
            return true;
        }
        ...
        foreach (['values', 'from', 'to', 'by', 'path'] as $attributeName) {
            if ($animation->hasAttribute($attributeName)
                && $this->hasUnsafeAnimationValue($animation->getAttribute($attributeName))
            ) {
                return true;
            }
        }
    ...
    private function hasSafeLocalHref(DOMElement $element): bool
    {
        foreach ($element->attributes as $attribute) {
            ...
            if (!str_starts_with($attribute->value, '#')
                || $attribute->value === '#'
                || preg_match('/[\x00-\x20]/', $attribute->value) === 1
            ) {
                return false;
            }
        }
    ...
    private function hasUnsafeAnimationValue(string $value): bool
    {
        ...
        if ($normalized === null
            || preg_match('/\b[a-z][a-z0-9+.-]*:/i', $normalized) === 1
            || str_contains($value, '\\')
            || str_contains($value, '/*')
            || str_contains($value, '*/')
        ) {
        ...
        $withoutLocalReferences = preg_replace(
            '~url\(\s*(?:"#[^"]+"|\'#[^\']+\'|#[^)\s]+)\s*\)~i',
            '',
            $value,
        );

        return $withoutLocalReferences === null
            || preg_match('/\burl\s*\(/i', $withoutLocalReferences) === 1;
    }
}
