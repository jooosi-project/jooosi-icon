...
class IconCacheKeyService
{
    ...
    {
        printf('<meta name="jooosi-icon-cache-key" content="%s" />' . "\n", esc_attr($this->get_key()));
    }
    /**
    ...
    public function add_inline_meta(array $scriptHandles): void
    {
        ...
        $script = sprintf('(()=>{const meta=document.createElement("meta");meta.name=%s;meta.content=%s;document.head.append(meta);})();', $metaName, $key);
