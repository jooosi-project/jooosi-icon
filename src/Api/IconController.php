...
final class IconController
{
     ...
     */
    #[Route('/item/(?P<prefix>[a-zA-Z0-9_-]+)/(?P<name>[a-zA-Z0-9_-]+)', 'GET', args: [
        'prefix' => [
            'validate_callback' => [self::class, 'validateIconPrefix'],
            'sanitize_callback' => 'sanitize_text_field',
            'required' => true,
        ],
        'name' => [
            'validate_callback' => [self::class, 'validateIconNamePart'],
            'sanitize_callback' => 'sanitize_text_field',
            'required' => true,
        ],
    ])]
    public function get_icon(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        if ($iconData === null) {
            return new WP_Error(
                'icon_not_found',
                sprintf('Icon "%s" not found', $iconName),
                ['status' => 404]
            );
        }
     ...
     * @return WP_REST_Response|WP_Error
     */
    #[Route('/item/(?P<prefix>[a-zA-Z0-9_-]+)/(?P<name>[a-zA-Z0-9_-]+)\.svg', 'GET', args: [
        'prefix' => [
            'validate_callback' => [self::class, 'validateIconPrefix'],
            'sanitize_callback' => 'sanitize_text_field',
            'required' => true,
     ...
     */
    #[Route('/search', 'GET', args: [
        'query' => [
            'validate_callback' => [self::class, 'validateSearchQuery'],
            'sanitize_callback' => 'sanitize_text_field',
            'required' => true,
        ],
    ])]
