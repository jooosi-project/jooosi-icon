...
final class Iconify
{
    ...
    public function __construct(private CacheItemPoolInterface $cache, private string $endpoint = self::API_ENDPOINT, private ?HttpClientInterface $httpClient = null, ?int $maxIconsQueryLength = null)
    {
    ...
    public function fetchIcons(string $prefix, array $names): array
    {
        ...
        $response = $this->http()->request('GET', \sprintf('/%s.json', $prefix), ['headers' => ['Accept' => 'application/json'], 'query' => ['icons' => strtolower($queryString)]]);
        if (200 !== $response->getStatusCode()) {
            throw new IconNotFoundException(\sprintf('The icon set "%s" does not exist on iconify.design.', $prefix));
        }
        $data = $response->toArray();
    ...
    {
        $response = $this->http()->request('GET', '/search', ['query' => ['query' => $query, 'prefix' => $prefix]]);
    ...
    private function sets(): \ArrayObject
    {
        ...
        $cacheItem = $this->cache->getItem('iconify-sets');
        if ($cacheItem->isHit()) {
            return $this->sets = $cacheItem->get();
        }
        $response = $this->http()->request('GET', '/collections');
        $data = new \ArrayObject($response->toArray());
        $cacheItem->set($data);
    ...
    private function http(): HttpClientInterface
    {
        ...
        if (!class_exists(HttpClient::class)) {
            throw new HttpClientNotInstalledException('You must install "symfony/http-client" to use icons from Iconify API. Try running "composer require symfony/http-client".');
        }
