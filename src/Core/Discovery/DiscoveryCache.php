...
final class DiscoveryCache
{
    ...
    public function store(\JooosiIcon\Core\Discovery\DiscoveryLocation $discoveryLocation, array $discoveries, string $source): void
    {
        ...
        try {
            ...
            $item->set(['data' => $data, 'manifest' => $manifest]);
            $this->cache->save($item);
        } catch (Throwable $e) {
            $this->logger->error('Failed to store cache', ['component' => 'DiscoveryCache', 'cacheKey' => $cacheKey, 'source' => $source, 'location' => $discoveryLocation->path, 'exception' => $e]);
        }
    }
    /**
    ...
    public function restore(\JooosiIcon\Core\Discovery\DiscoveryLocation $discoveryLocation, string $source): ?array
    {
        ...
        try {
            ...
            $this->logger->error('Failed to restore cache', ['component' => 'DiscoveryCache', 'cacheKey' => $cacheKey, 'source' => $source, 'location' => $discoveryLocation->path, 'exception' => $e]);
            return null;
        }
    }
    ...
    private function createManifest(\JooosiIcon\Core\Discovery\DiscoveryLocation $discoveryLocation): array
    {
        ...
        try {
            $iterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($discoveryLocation->path, RecursiveDirectoryIterator::SKIP_DOTS));
            ...
            $this->logger->warning('Failed to create manifest', ['component' => 'DiscoveryCache', 'location' => $discoveryLocation->path, 'exception' => $e]);
        }
    ...
    private function isManifestStale(\JooosiIcon\Core\Discovery\DiscoveryLocation $discoveryLocation, array $cachedManifest): bool
    {
        ...
        try {
            ...
            $this->logger->warning('Failed to check manifest staleness, considering stale', ['component' => 'DiscoveryCache', 'location' => $discoveryLocation->path, 'exception' => $e]);
            return \true;
        }
    }
}
