...
final class ControllerDiscovery implements \JooosiIcon\Core\Discovery\Discovery
{
    ...
    private function registerRestRoutes(): void
    {
        add_action('rest_api_init', function () {
            foreach ($this->routes as $route) {
                register_rest_route($route['namespace'], $route['path'], ['methods' => $route['methods'], 'callback' => $this->createRouteCallback($route), 'permission_callback' => $this->createPermissionCallback($route), 'args' => $route['args']]);
            }
        });
    }
    ...
    private function createPermissionCallback(array $route): callable
    {
        return function () use ($route) {
            ...
            if (in_array('auth', $route['middleware'], \true)) {
                return is_user_logged_in();
            }
            if (in_array('admin', $route['middleware'], \true)) {
