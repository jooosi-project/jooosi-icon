...
class JOOOSI_ICON
{
    ...
    public static function resolve_upload_location(?array $uploads = null): array
    {
        ...
        if (\is_dir($canonical_dir) || $migrated || !\is_dir($legacy_dir)) {
            return ['basedir' => $canonical_dir, 'baseurl' => $base_url . self::UPLOAD_DIR, 'relative' => self::UPLOAD_DIR, 'migrated' => $migrated, 'legacy' => \false];
        }
        return ['basedir' => $legacy_dir, 'baseurl' => $base_url . self::LEGACY_UPLOAD_DIR, 'relative' => self::LEGACY_UPLOAD_DIR, 'migrated' => \false, 'legacy' => \true];
    }
}
\class_alias('JooosiIconDeps\JOOOSI_ICON', 'JOOOSI_ICON', \false);
