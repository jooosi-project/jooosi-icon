...
update('jooosi-icon.php', [[/( \* Version:\s+)\d+\.\d+\.\d+/, `$1${version}`]]);
...
const repositoryUrl = 'https://github.com/jooosi-project/jooosi-icon';
const comparisonLink = changelog.match(
  /^\[unreleased\]: https:\/\/github\.com\/jooosi-project\/jooosi-icon\/compare\/(v?\d+\.\d+\.\d+)\.\.\.HEAD$/m,
);
...
const releaseFiles = ['CHANGELOG.md', 'constant.php', 'jooosi-icon.php', 'package.json', 'readme.txt'];
